...
@pytest.fixture
def org_id(db):
    """Every ctid_connection belongs to an org (NOT NULL since migration 005),
    so a token store test needs a real one to hang its grants off."""
    with psycopg.connect(db, autocommit=True) as conn:
        (org_id,) = conn.execute(
            "INSERT INTO orgs (name) VALUES ('Token Store Org') RETURNING id"
        ).fetchone()
    return org_id
...
def test_save_grant_encrypts_tokens_at_rest(db, org_id):
    store = make_store(db)
    cid = store.save_grant("access-1", "refresh-1", NOW + timedelta(days=30), org_id)
...
def test_get_roundtrips(db, org_id):
    store = make_store(db)
    cid = store.save_grant("a", "r", NOW + timedelta(days=30), org_id)
    pair = store.get(cid)
    assert (pair.access_token, pair.refresh_token, pair.status) == ("a", "r", "active")


def test_rotate_persists_new_refresh_token(db, org_id):
    # spec: refresh token rotates on every refresh; new one MUST be persisted
    store = make_store(db)
    cid = store.save_grant("a1", "r1", NOW + timedelta(days=30), org_id)
...
def test_due_for_refresh_under_25_days_remaining(db, org_id):
    store = make_store(db)
    due = store.save_grant("a", "r", NOW + timedelta(days=24), org_id)
    fresh = store.save_grant("a", "r", NOW + timedelta(days=26), org_id)
    assert store.due_for_refresh(NOW) == [due]
    assert fresh not in store.due_for_refresh(NOW)


def test_mark_status(db, org_id):
    store = make_store(db)
    cid = store.save_grant("a", "r", NOW + timedelta(days=30), org_id)
...
def test_rotate_succeeds_with_valid_connection_id(db, org_id):
    store = make_store(db)
    cid = store.save_grant("a1", "r1", NOW + timedelta(days=30), org_id)
...
def test_mark_succeeds_with_valid_connection_id(db, org_id):
    store = make_store(db)
    cid = store.save_grant("a", "r", NOW + timedelta(days=30), org_id)
...
def test_due_for_refresh_at_exactly_threshold_not_due(db, org_id):
    # Spec: expires_at - now < REFRESH_THRESHOLD (strict <, not <=)
    # So at exactly 25 days, NOT due
    store = make_store(db)
    exactly_threshold = store.save_grant("a", "r", NOW + timedelta(days=25), org_id)
    assert store.due_for_refresh(NOW) == []
    assert exactly_threshold not in store.due_for_refresh(NOW)


def test_due_for_refresh_excludes_invalid_status(db, org_id):
    store = make_store(db)
    # Connection that would be due except for status
    due_id = store.save_grant("a", "r", NOW + timedelta(days=24), org_id)
    store.mark(due_id, "invalid")
    # Even though expiry qualifies (< 25 days), status='invalid' excludes it
    assert store.due_for_refresh(NOW) == []


def test_due_for_refresh_excludes_refresh_failed_status(db, org_id):
    store = make_store(db)
    # Connection that would be due except for status
    due_id = store.save_grant("a", "r", NOW + timedelta(days=24), org_id)
