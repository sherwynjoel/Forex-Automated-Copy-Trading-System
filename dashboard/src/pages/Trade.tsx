...
export default function Trade() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        setMargin(await api<MarginEstimate>(
          `/api/accounts/${accountId}/margin-estimate` +
  ...
  }, [accountId, ticket.symbol, ticket.volumeLots])
  ...
  const loadChart = useCallback(async () => {
    ...
    try {
      setChart(await api<Trendbars>(
        `/api/accounts/${accountId}/trendbars` +
        `?symbol=${encodeURIComponent(ticket.symbol)}&period=${chartPeriod}` +
        `&from=${from}&to=${to}`))
    } catch {
      setChart(null)
    }
  }, [accountId, ticket.symbol, chartPeriod])
