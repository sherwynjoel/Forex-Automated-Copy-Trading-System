...
export default function Accounts() {
  ...
  const handleRoleChange = async (accountId: number, newRole: string) => {
    try {
      ...
      if (errorCode === '409') {
        ...
        setRoleErrors((prev) => ({ ...prev, [accountId]: `Error: ${errorCode}` }))
      }
    }
  }
  ...
  const handleMultiplierChange = async (accountId: number, newMultiplier: string) => {
    ...
    try {
      ...
      const errorCode = err instanceof Error ? err.message : 'Unknown error'
      setMultiplierErrors((prev) => ({ ...prev, [accountId]: `Error: ${errorCode}` }))
    }
  }
          ...
          <table className="w-full border-collapse border border-gray-300">
            ...
            <tbody>
              {accounts.map((account) => (
                <tr key={account.ctid_trader_account_id} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-300 px-4 py-2 text-sm">
                    <input
                      ...
                      onChange={() => {
                        handleRoleChange(account.ctid_trader_account_id, account.role)
                        // TODO: handle enabled toggle
                      }}
                      className="w-4 h-4"
                    />
                  </td>
