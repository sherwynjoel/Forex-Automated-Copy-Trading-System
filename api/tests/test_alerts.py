...
The alerter is deliberately INSTANCE-WIDE, not per-org (one API key, one
recipient, configured from the environment): it is operator-level alerting
across every desk in the deployment. So it consumes every event the
broadcaster fetches whatever org stamped it, including the NULL-org
infrastructure events no WebSocket ever receives. Per-org recipients are
future work; see the README's alerts section."""
import asyncio
import json
import os
import time
...
def test_alerts_are_instance_wide_across_orgs():
    """Every org's events reach the same operator inbox, and an event that
    belongs to NO org (infrastructure events, which the org-keyed
    broadcaster delivers to no socket at all) still alerts."""
    recorded = []
    alerter = _make_alerter(recorded)

    assert _run(alerter.consider(_event(org_id=1, account_id=101))) is True
    assert _run(alerter.consider(_event(org_id=2, account_id=202))) is True
    assert _run(alerter.consider(
        _event("auth", "error", "token_refresh_failed",
               account_id=None, org_id=None))) is True
