...
def _require_account(conn: psycopg.Connection, account_id: int) -> None:
    exists = conn.execute(
...
def create_insights_router() -> APIRouter:
    """Create router for insight endpoints."""
    ...
    async def overview(
        ...
        """The Overview page's DB-derived stats: fleet counts, copies made
        today, yesterday's portfolio snapshot (for the vs-yesterday delta),
        and the recent-copy feed. Live equity/positions come from
        /api/state; this endpoint covers what only the database knows."""
        accounts = conn.execute(
            "SELECT role, enabled, status FROM accounts").fetchall()
