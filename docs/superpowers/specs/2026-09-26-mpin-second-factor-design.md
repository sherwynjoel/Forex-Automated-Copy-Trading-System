...
| `POST /api/me/mpin` `{current_mpin, mpin, mpin_confirm}` | full | Verify the current MPIN (401 `Invalid MPIN`, counting toward the same lock), set the new hash and `mpin_set_at`, audit `mpin_changed`. 204. No new cookie needed. |
